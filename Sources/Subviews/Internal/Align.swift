...
// This is slightly modified version of an amazing library Aligh
// by Alexander Grebenyuk (github.com/kean)
...
internal extension LayoutAnchors where Base: LayoutItem {
    ...
    var edges: AnchorCollectionEdges { AnchorCollectionEdges(item: base, axis: nil) }
    var horizontalEdges: AnchorCollectionEdges { AnchorCollectionEdges(item: base, axis: .horizontal) }
    var verticalEdges: AnchorCollectionEdges { AnchorCollectionEdges(item: base, axis: .vertical) }
...
internal struct AnchorCollectionEdges {
    ...
    internal func absolute() -> AnchorCollectionEdges {
        AnchorCollectionEdges(item: item, isAbsolute: true, axis: axis)
    }
    ...
    @available(*, deprecated, message: "Use .horizontal or .vertical before calling .pin instead of specifying axis")
    @discardableResult internal func pin(to item2: LayoutItem? = nil, insets: EdgeInsets = .zero, axis: Axis?, alignment: Alignmment = .fill) -> [NSLayoutConstraint] {
        if let axis = axis {
            return AnchorCollectionEdges(item: item, isAbsolute: isAbsolute, axis: axis).pin(to: item2, insets: insets, alignment: alignment)
        } else {
            return pin(to: item2, insets: insets, alignment: alignment)
        }
    }
}
